...
/** Typed numerical and independent reference paths for an isobaric ideal-gas process. */
public final class IdealGasIsobaricModule implements PhysicsModule<IdealGasIsobaricModule.Parameters> {
    ...
    public Parameters bind(CanonicalQuantityBag quantities) {
        if (quantities == null) {
            throw new IllegalArgumentException("Canonical isobaric ideal-gas quantities are required");
        }
    ...
    public SolverOutput solve(Parameters parameters, SimulationClock clock) {
        ...
        if (clock == null) {
            throw new IllegalArgumentException("Isobaric ideal-gas simulation clock is required");
        }

        // Numerical path: apply Charles' law at the fixed initial pressure,
        // then calculate boundary work from P * delta-V.
    ...
    public AnalyticalPoint referenceAt(Parameters parameters, double timeSeconds) {
        ...
        // Independent oracle: derive nR from the initial state, use the
        // ideal-gas equation to get final volume, and use W = nR * delta-T.
        // It does not call the numerical path's temperature-ratio or P*delta-V
        // calculations.
        double logMolarScale = Math.log(parameters.initialPressure())
                + Math.log(parameters.initialVolume())
                - Math.log(parameters.initialTemperature());
        double molarScale = Math.exp(logMolarScale);
        double finalVolume = Math.exp(logMolarScale + Math.log(parameters.finalTemperature())
                - Math.log(parameters.initialPressure()));
        double finalPressure = parameters.initialPressure();
    ...
    private static void requireParameters(Parameters parameters) {
        if (parameters == null) {
            throw new IllegalArgumentException("Isobaric ideal-gas parameters are required");
        }
    }

    private static void requireFinite(String outputKey, double value) {
        if (!Double.isFinite(value)) {
            throw new IllegalArgumentException("Isobaric ideal-gas process produced non-finite " + outputKey);
        }
    }
    ...
    private static void requirePositiveFinite(String outputKey, double value) {
        ...
        if (value <= 0.0) {
            throw new IllegalArgumentException("Isobaric ideal-gas process produced non-positive " + outputKey);
        }
    }
