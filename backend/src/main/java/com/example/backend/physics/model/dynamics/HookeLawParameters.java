...
public record HookeLawParameters(double springConstant, double displacement) {
    public static HookeLawParameters from(JsonNode specification, Map<String, Double> overrides) {
        ...
        if (!(k > 0) || !Double.isFinite(x)) throw new IllegalArgumentException("Spring constant and displacement are invalid");
        return new HookeLawParameters(k, x);
    }
    public double restoringForce() { return -springConstant * displacement; }
    public double elasticPotentialEnergy() { return 0.5 * springConstant * displacement * displacement; }
}
