...
public class MechanicsFoundationReferenceSolver implements ReferenceSolver {
    ...
    @Override public AnalyticalPoint solve(JsonNode specification, Map<String, Double> overrides, double timeSeconds) {
        ...
        switch (model) {
            ...
            case "hooke_law" -> { HookeLawParameters p = HookeLawParameters.from(PhysicsValues.bag(specification, overrides)); values.put("restoringForce", p.restoringForce()); values.put("elasticPotentialEnergy", p.elasticPotentialEnergy()); }
            ...
            case "hydrostatics" -> { HydrostaticsParameters p = HydrostaticsParameters.from(PhysicsValues.bag(specification, overrides)); values.put("gaugePressure", p.gaugePressure()); values.put("absolutePressure", p.absolutePressure()); values.put("buoyantForce", p.buoyantForce()); }
