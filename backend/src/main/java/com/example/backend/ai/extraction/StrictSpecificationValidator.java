...
public final class StrictSpecificationValidator {
    private static final Set<String> ROOT_FIELDS = Set.of("contractVersion", "schemaVersion", "topic", "schemaId",
            "objects",
    ...
    private static final Set<String> RELATION_FIELDS = Set.of("type", "subject", "object", "value", "unit",
            "sourceText");
    ...
    public static void validate(JsonNode root) {
        ...
        if (!"1.0".equals(root.path("contractVersion").asText()))
            fail("contractVersion must be 1.0");
        ...
        if (root.has("visualBindings")) {
            ...
            for (JsonNode binding : root.path("visualBindings")) {
                rejectUnknown(binding, Set.of("targetId", "entityId", "assetId", "match", "visualDifference"),
                        "visual binding");
                requiredText(binding, "targetId");
                if (!targets.add(binding.path("targetId").asText()))
                    fail("duplicate visual target");
        ...
        if (!root.path("endCondition").isObject())
            fail("endCondition must be an object");
        validateEndCondition(root.path("endCondition"));
    }

    /**
     * Verify candidate membership and, when a unit catalog is supplied, reject
     * units before the JSON tree is bound to extraction POJOs.
     */
    public static SchemaCandidate validateCandidateMembership(JsonNode root, SchemaRoutingDecision decision,
        ...
        return validateCandidateMembership(root, decision, unitNormalizer, false);
    }
    ...
    public static SchemaCandidate validateCandidateMembership(JsonNode root, SchemaRoutingDecision decision,
        ...
        if (decision.status() == SchemaRoutingDecision.Status.SELECTED) {
            SchemaCandidate selected = decision.selectedCandidate().orElseThrow(() -> new IllegalArgumentException(
                    "Invalid AI specification: selected route has no pinned candidate"));
        ...
        for (QuantityProjection quantity : contract.requiredQuantities())
            addAcceptedNames(quantity, quantityNames);
        ...
        for (JsonNode quantity : root.path("quantities")) {
            ...
            if (!quantityNames.contains(name))
                fail("quantity name is outside the selected candidate contract");
            String canonicalName = canonicalQuantityName(contract, name);
            if (canonicalName == null)
                fail("quantity name is ambiguous in the selected candidate contract");
            ...
            if (unitNormalizer != null)
                validateQuantityUnit(quantity, canonicalName,
        ...
        if (!contract.entityTypes().isEmpty()) {
            for (JsonNode object : root.path("objects")) {
                ...
                if (entity == null)
                    fail("object type is outside the selected entity contract");
                ...
                for (JsonNode quantity : object.path("quantities")) {
                    ...
                    if (!accepted.contains(name))
                        fail("entity quantity name is outside the selected candidate contract");
                    String canonical = canonicalQuantityName(entity.requiredQuantities(), entity.optionalQuantities(),
                            name);
            ...
            for (EntityTypeProjection entity : contract.entityTypes()) {
                ...
                if (count < entity.minCount() || (!capacityConflictPending && count > entity.maxCount())) {
        ...
        if (!capabilities.isEmpty()) {
            ...
            if (!supported)
                fail("endCondition type is outside the selected candidate capabilities");
        }
        ...
        for (JsonNode ambiguity : root.path("ambiguities")) {
            ...
            if (fieldPath.startsWith("quantities.")) {
                ...
                if (!required && !optional)
                    fail("quantity ambiguity fieldPath is outside the selected candidate contract");
                if (optional)
                    fail("ambiguity must not request an optional quantity");
            ...
            if (fieldPath.startsWith("objects.") && entityMarker > "objects.".length()) {
                ...
                if (entity == null)
                    fail("entity quantity ambiguity references an unknown object");
                ...
                if (!required && !optional)
                    fail("entity quantity ambiguity is outside the selected candidate contract");
                if (optional)
                    fail("ambiguity must not request an optional entity quantity");
            ...
            if ("schemaId".equals(fieldPath) || "schema.schemaId".equals(fieldPath)) {
                if ("JEV_CAPACITY_EXCEEDED".equals(decision.reasonCode())) {
                    fail("capacity conflicts must use a compatibility fieldPath, not schemaId");
                }
                List<String> options = new java.util.ArrayList<>();
                ambiguity.path("options").forEach(option -> options.add(option.asText()));
                if (decision.status() != SchemaRoutingDecision.Status.AMBIGUOUS || options.isEmpty()
                        || options.stream().anyMatch(
                                option -> !candidateLabels.contains(option) && !candidateIds.contains(option))) {
    ...
    private static String canonicalQuantityName(List<QuantityProjection> required,
        ...
        for (QuantityProjection quantity : required) {
            if (accepts(quantity, suppliedName))
                matches.add(quantity.key());
        }
        ...
        for (JsonNode item : values) {
            ...
            if (quantities != null && !quantities.isNull()) {
                if (!quantities.isArray())
                    fail("object.quantities must be an array");
    ...
    private static void validateQuantities(JsonNode values) {
        ...
        for (JsonNode item : values) {
            ...
            if (!names.add(item.path("name").asText()))
                fail("duplicate quantity name");
            requiredNumber(item, "value");
            requiredText(item, "originalUnit");
            optionalNumber(item, "confidence");
            optionalText(item, "symbol");
            optionalText(item, "sourceText");
        }
    }
    ...
    private static void validateAmbiguities(JsonNode values) {
        ...
        for (JsonNode item : values) {
            ...
            requiredText(item, "fieldPath");
            requiredText(item, "question");
            if (!codes.add(item.path("code").asText()))
                fail("duplicate ambiguity code");
            if (!paths.add(item.path("fieldPath").asText()))
                fail("duplicate ambiguity fieldPath");
            JsonNode options = item.get("options");
            if (options == null || !options.isArray() || options.size() > MAX_ITEMS)
                fail("ambiguity.options must be an array");
            for (JsonNode option : options)
                if (!option.isTextual() || option.asText().length() > MAX_TEXT)
                    fail("ambiguity option must be text");
        }
    }
    ...
    private static void validateResolutionDecisions(JsonNode values) {
        if (values == null)
            return;
        if (!values.isArray() || values.size() > MAX_ITEMS)
            fail("resolutionDecisions must be a bounded array");
        Set<String> codes = new HashSet<>();
        ...
        for (JsonNode decision : values) {
            ...
            if (omittedIds == null || !omittedIds.isArray())
                fail("resolution decision omittedObjectIds must be an array");
            Set<String> ids = new HashSet<>();
    ...
    private static void rejectUnknown(JsonNode object, Set<String> allowed, String label) {
        ...
        while (names.hasNext()) {
            ...
            if (!allowed.contains(field))
                fail("Unknown field '" + field + "' in " + label);
        }
    }
    ...
    private static void requiredNumber(JsonNode object, String key) {
        JsonNode value = object.get(key);
        if (value == null || !value.isNumber() || !finite(value))
            fail(key + " must be a finite JSON number");
    }

    private static void optionalNumber(JsonNode object, String key) {
        JsonNode value = object.get(key);
        if (value != null && !value.isNull() && (!value.isNumber() || !finite(value)))
            fail(key + " must be a finite JSON number or null");
    }

    private static void array(JsonNode object, String key) {
        if (!object.path(key).isArray())
    ...
    private static void limit(JsonNode value, String label) {
        if (value.size() > MAX_ITEMS)
            fail(label + " exceeds the item limit");
    }
