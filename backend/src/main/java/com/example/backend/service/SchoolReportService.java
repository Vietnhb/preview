...
public class SchoolReportService {
    ...
    public Summary summary(UUID schoolId) {
        ...
        long teachers = users.findBySchoolIdAndRoleName(schoolId, "TEACHER").size();
        long students = users.countActiveStudents(schoolId);
        long managers = users.findBySchoolIdAndRoleName(schoolId, "SCHOOL_MANAGER").size();
    ...
    public ImportResult importUsers(UUID schoolId, MultipartFile file) {
        ...
        List<ImportRow> rows = new ArrayList<>(); int imported = 0; int rowNumber = 1;
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    if (!role.equals("TEACHER") && !role.equals("STUDENT")) throw new ApiException(HttpStatus.BAD_REQUEST, "Role phải là TEACHER hoặc STUDENT");
