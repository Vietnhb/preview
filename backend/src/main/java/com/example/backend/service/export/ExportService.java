package com.example.backend.service.export;
...
public class ExportService {
    ...
    public byte[] offlineReplayHtml(java.util.UUID specificationId) {
        ...
        String html = """
                ...
                <canvas id="scene" width="960" height="420"></canvas><input id="timeline" type="range" min="0" value="0" step="1"><p><button id="play">ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ PhÃƒÆ’Ã‚Â¡t</button></p><div class="meta" id="meta"></div>
                ...
                title.textContent=b.specification.schemaId;timeline.max=Math.max(0,times.length-1);meta.innerHTML=`<span>Schema ${b.specification.schemaVersion}</span><span>Validation ${s.validationPassed?'PASSED':'FAILED'}</span><span>${times.length} mÃƒÂ¡Ã‚ÂºÃ‚Â«u</span>`;
                ...
                timeline.oninput=draw;play.onclick=()=>{if(timer){clearInterval(timer);timer=null;play.textContent='ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ PhÃƒÆ’Ã‚Â¡t';return}play.textContent='ÃƒÂ¢Ã‚ÂÃ‚Â¸ DÃƒÂ¡Ã‚Â»Ã‚Â«ng';timer=setInterval(()=>{timeline.value=(+timeline.value+1)%%Math.max(1,times.length);draw()},50)};draw();
                </script></body></html>
                """
                .formatted(payload);
        ...
        try {
            ...
            String html = """
                    ...
                    *{box-sizing:border-box}body{margin:0;background:#eef4fb;color:#15233b;font-family:Inter,system-ui,sans-serif}.deck{width:min(1100px,100%%);margin:auto;padding:28px}.slide{min-height:620px;margin:0 0 22px;padding:54px;border:1px solid #d8e2ef;border-radius:24px;background:#fff;box-shadow:0 8px 28px #233d5c14;page-break-after:always}.kicker{color:#3569b8;font-size:12px;font-weight:800;letter-spacing:.12em}.title{font-size:46px;line-height:1.08;margin:18px 0 14px}.muted{color:#667892;line-height:1.7}.metric-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:36px}.metric{padding:20px;border-radius:16px;background:#f1f6fd}.metric strong{display:block;margin-top:8px;font-size:28px;color:#2563eb}.table{width:100%%;border-collapse:collapse;margin-top:26px}.table th,.table td{text-align:left;padding:13px 10px;border-bottom:1px solid #e1e8f0}.table th{color:#5c7190;font-size:12px}.result{padding:18px;border-radius:16px;background:#f5f8fc;margin-top:14px}.controls{display:flex;gap:10px;flex-wrap:wrap}.controls span{padding:8px 12px;border-radius:999px;background:#e8f1fd;color:#244e88}@media print{body{background:#fff}.deck{padding:0}.slide{border:0;box-shadow:none;margin:0;border-radius:0}}
                    ...
                    const b=JSON.parse(document.getElementById('bundle').textContent),s=b.simulation,q=b.specification,deck=document.getElementById('deck');const params=Object.entries(s.parameters||{}).map(([k,v])=>`<span>${k}: ${v}</span>`).join('');const quantities=(q.quantities||[]).map(x=>`<tr><td>${x.name}</td><td>${x.normalizedValue}</td><td>${x.normalizedUnit}</td></tr>`).join('');const values=Object.entries(s.values||{}).map(([k,v])=>`<div class="result"><strong>${k}</strong><br>Ãƒâ€žÃ‚ÂiÃƒÂ¡Ã‚Â»Ã†â€™m Ãƒâ€žÃ¢â‚¬ËœÃƒÂ¡Ã‚ÂºÃ‚Â§u: ${v[0]??'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â'} Ãƒâ€šÃ‚Â· Ãƒâ€žÃ‚ÂiÃƒÂ¡Ã‚Â»Ã†â€™m cuÃƒÂ¡Ã‚Â»Ã¢â‚¬Ëœi: ${v[v.length-1]??'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â'} Ãƒâ€šÃ‚Â· ${v.length} mÃƒÂ¡Ã‚ÂºÃ‚Â«u</div>`).join('');deck.innerHTML=`<section class="slide"><span class="kicker">PHYSLIVE / SIMULATION SLIDES</span><h1 class="title">${q.schemaId||'MÃƒÆ’Ã‚Â´ phÃƒÂ¡Ã‚Â»Ã‚Âng vÃƒÂ¡Ã‚ÂºÃ‚Â­t lÃƒÆ’Ã‚Â½'}</h1><p class="muted">BÃƒÂ¡Ã‚Â»Ã¢â€žÂ¢ slide Ãƒâ€žÃ¢â‚¬ËœÃƒâ€ Ã‚Â°ÃƒÂ¡Ã‚Â»Ã‚Â£c tÃƒÂ¡Ã‚ÂºÃ‚Â¡o tÃƒÂ¡Ã‚Â»Ã‚Â« specification Ãƒâ€žÃ¢â‚¬ËœÃƒÆ’Ã‚Â£ kiÃƒÂ¡Ã‚Â»Ã†â€™m chÃƒÂ¡Ã‚Â»Ã‚Â©ng Ãƒâ€žÃ¢â‚¬ËœÃƒÂ¡Ã‚Â»Ã¢â€žÂ¢c lÃƒÂ¡Ã‚ÂºÃ‚Â­p.</p><div class="metric-grid"><div class="metric">Validation<strong>${s.validationPassed?'PASS':'FAIL'}</strong></div><div class="metric">Schema<strong>${q.schemaVersion||'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â'}</strong></div><div class="metric">Samples<strong>${(s.time||[]).length}</strong></div></div></section><section class="slide"><span class="kicker">01 / INPUT</span><h2>CÃƒÆ’Ã‚Â¡c Ãƒâ€žÃ¢â‚¬ËœÃƒÂ¡Ã‚ÂºÃ‚Â¡i lÃƒâ€ Ã‚Â°ÃƒÂ¡Ã‚Â»Ã‚Â£ng Ãƒâ€žÃ¢â‚¬ËœÃƒÂ¡Ã‚ÂºÃ‚Â§u vÃƒÆ’Ã‚Â o</h2><table class="table"><thead><tr><th>TÃƒÆ’Ã‚Âªn</th><th>GiÃƒÆ’Ã‚Â¡ trÃƒÂ¡Ã‚Â»Ã¢â‚¬Â¹ chuÃƒÂ¡Ã‚ÂºÃ‚Â©n hÃƒÆ’Ã‚Â³a</th><th>Ãƒâ€žÃ‚ÂÃƒâ€ Ã‚Â¡n vÃƒÂ¡Ã‚Â»Ã¢â‚¬Â¹</th></tr></thead><tbody>${quantities}</tbody></table><div class="controls">${params}</div></section><section class="slide"><span class="kicker">02 / OUTPUT</span><h2>KÃƒÂ¡Ã‚ÂºÃ‚Â¿t quÃƒÂ¡Ã‚ÂºÃ‚Â£ mÃƒÆ’Ã‚Â´ phÃƒÂ¡Ã‚Â»Ã‚Âng</h2><p class="muted">ThÃƒÂ¡Ã‚Â»Ã‚Âi gian: ${(s.time||[])[0]??0} ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ ${(s.time||[]).slice(-1)[0]??0} s</p>${values}</section>`;
                    </script></body></html>
                    """.formatted(payload);
            return html.getBytes(StandardCharsets.UTF_8);
        } catch (Exception exception) {
