...
public class ExportService {
    ...
    public byte[] simulationCsv(java.util.UUID specificationId) {
        ...
        for (String column : columns) csv.append(',').append(column);
        csv.append('\n');
    ...
    public byte[] simulationPdf(java.util.UUID specificationId) {
        ...
        try (PDDocument document = new PDDocument(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            try (PDPageContentStream stream = new PDPageContentStream(document, page)) {
                stream.beginText(); stream.setFont(new PDType1Font(Standard14Fonts.FontName.HELVETICA_BOLD), 16); stream.newLineAtOffset(50, 750); stream.showText("PhysLive simulation report"); stream.endText();
                stream.beginText(); stream.setFont(new PDType1Font(Standard14Fonts.FontName.HELVETICA), 10); stream.newLineAtOffset(50, 720); stream.showText("Schema: " + response.schemaId()); stream.newLineAtOffset(0, -16); stream.showText("Validation: " + response.validationPassed()); stream.newLineAtOffset(0, -16); stream.showText("Samples: " + response.time().size()); stream.newLineAtOffset(0, -24);
                int rows = Math.min(response.time().size(), 32); List<String> columns = new ArrayList<>(response.values().keySet());
                for (int i = 0; i < rows; i++) { StringBuilder line = new StringBuilder("t=").append(String.format(java.util.Locale.ROOT, "%.3f", response.time().get(i))); for (String column : columns) { List<Double> values = response.values().get(column); if (values != null && i < values.size()) line.append(" ").append(column).append("=").append(String.format(java.util.Locale.ROOT, "%.3f", values.get(i))); } stream.showText(line.toString()); stream.newLineAtOffset(0, -14); }
                stream.endText();
            }
            document.save(output); return output.toByteArray();
        } catch (Exception ex) { throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "Could not export PDF report"); }
    }
    ...
    public byte[] offlineReplayHtml(java.util.UUID specificationId) {
        ...
        if (!"PASSED".equals(specification.getValidationStatus())) {
            throw new ApiException(HttpStatus.CONFLICT, "Only validated simulations can be exported for offline replay");
        }
        SimulationResponse response = simulationService.get(simulation.getId());
        ...
        String payload;
        try { payload = objectMapper.writeValueAsString(bundle).replace("</", "<\\/"); }
        catch (Exception exception) { throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "Could not create offline replay"); }
        String html = """
    ...
    private String resolveSchema(Specification specification) {
        if (specification.getSchemaId() != null && !specification.getSchemaId().isBlank()) return specification.getSchemaId();
