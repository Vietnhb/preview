-- Re-apply the runtime contract after the legacy Flyway baseline. Earlier
-- guarded migrations could be marked as applied before Hibernate created the
-- corresponding tables. Every operation here is intentionally idempotent.
DO $$
BEGIN
    ...
    IF to_regclass('simulations') IS NOT NULL THEN
        ...
        -- The immutable SimulationRun relation replaces the legacy validation
        -- entity. Dropping its FK column also removes the generated FK safely.
        ALTER TABLE simulations DROP COLUMN IF EXISTS validation_run_id;
    END IF;
