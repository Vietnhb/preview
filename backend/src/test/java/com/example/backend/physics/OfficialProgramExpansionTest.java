...
class OfficialProgramExpansionTest {
        ...
        void dampedForcedOscillationMatchesReference() {
                ...
                assertEquals(checkpoint.values().get("displacement"), output.values().get("displacement").get(13),
                                1e-12);
                ...
                var critical = new AdvancedOscillationSolver().solve(
                                specification.deepCopy().put("damping_coefficient", 4), Map.of(), 1, .1);
                var over = new AdvancedOscillationSolver().solve(
                                specification.deepCopy().put("damping_coefficient", 6), Map.of(), 1, .1);
                assertEquals(0.1, critical.values().get("displacement").get(0), 1e-12);
                assertEquals(0.1, over.values().get("displacement").get(0), 1e-12);
        }

        @Test
        void dampingCoefficientUsesItsDeclaredKgPerSecondDimension() {
                ObjectNode specification = mapper.createObjectNode().put("model", "damped_forced_oscillation")
                                .put("mass", 2).put("spring_constant", 8).put("damping_coefficient", 8)
        ...
        void phaseChangeHasLatentHeatPlateaus() {
                ...
                assertEquals(reference.values().get("liquidFraction"), output.values().get("liquidFraction").get(334),
                                1e-12);
                assertEquals(1, new PhaseChangeReferenceSolver().solve(specification, Map.of(), 700).values()
                                .get("liquidFraction"));
        }
        ...
        void medicalModelsMatchClosedForm() {
                ...
                assertEquals(xrayOutput.values().get("halfValueLayer").get(0),
                                new MedicalImagingReferenceSolver().solve(xray, Map.of(), 0).values()
                                                .get("halfValueLayer"),
                                1e-12);
                ...
                assertEquals(1, new MedicalImagingReferenceSolver().solve(ct, Map.of(), 0).values().get("lineIntegral"),
                                1e-12);
        ...
        void deBroglieDiffractionMatchesReference() {
                ...
                assertEquals(reference.values().get("diffractionAngle"), output.values().get("diffractionAngle").get(0),
                                1e-12);
        ...
        void momentBalanceUsesPerpendicularComponents() {
                ...
                assertEquals(output.values().get("equilibriumResidual").get(0),
                                new MomentEquilibriumReferenceSolver().solve(moment, Map.of(), 0).values()
                                                .get("equilibriumResidual"),
                                1e-12);
        }
