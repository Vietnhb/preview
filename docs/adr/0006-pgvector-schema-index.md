...
Migration: `backend/src/main/resources/db/migration/V8__add_hybrid_schema_search.sql`, `V9__add_solver_binding_checksum.sql`, and `V10__add_schema_search_projection_version.sql`. JDBC and cosine retrieval: `backend/src/main/java/com/example/backend/schema/routing/vector/PgVectorSchemaEmbeddingStore.java` and `PgVectorSchemaRetriever.java`. Readiness: `SchemaRoutingHealthIndicator.java`.

## Known gaps and migration blocker

- Testcontainers coverage now verifies V8–V12, pgvector storage, the application cosine query, projection-version freshness, typed simulation-run contract identity, legacy ambiguity/reviewer table repair, and a preserved prior-schema fixture from a V7 baseline. The current Docker-enabled run passes.
- The full V1–V12 chain still cannot run on a truly empty database without the documented Hibernate-first sequence: V4 first alters the Hibernate-owned `assignment_submissions` table without an existence guard; V5–V7 also expect Hibernate-created tables. V8–V12 no longer depend on `schema_versions` or the ambiguity/reviewer tables existing before Hibernate. Resolve the legacy bootstrap sequencing without changing applied migrations, then prove a supported direct clean path if that is required by deployment.
- The embedding freshness check compares `search_text` as well as schema checksum/provider/model/dimension/projection version, so projection changes trigger re-embedding.
