...
export default function SchoolReports() {
  ...
  return <div className="admin-content school-reports">
    <div className="school-report-heading">
      <div><p className="admin-eyebrow">DỮ LIỆU TRƯỜNG</p><h1>Báo cáo & nhập dữ liệu</h1><p>Tổng hợp lớp học, tài khoản và lịch sử sử dụng AI.</p></div>
      <button type="button" className="admin-secondary-button" onClick={() => void load()} disabled={loading}><LearningIcon name="refresh" />{loading ? "Đang tải…" : "Làm mới"}</button>
    </div>
    ...
    {loading && !summary ? <div className="admin-loading"><span className="admin-loading-spinner" /><p>Đang tải báo cáo…</p></div> : summary && <>
      <section className="admin-stats-grid school-summary-grid" aria-label="Tổng quan trường">
      ...
      <section className="admin-panel school-import-panel">
        ...
        <p className="school-csv-format">Dòng tiêu đề: <code>{classCsvColumns}</code>. Vai trò nhận <code>TEACHER</code> hoặc <code>STUDENT</code>.</p>
