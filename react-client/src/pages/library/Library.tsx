...
export default function Library() {
  ...
  useEffect(() => {
    const requests = user?.role === "TEACHER" ? [personalLibrary(), library()] : [Promise.resolve([]), library()];
  ...
  return <main className="main">
    ...
    {user?.role === "TEACHER" && <section className="card library-section"><div className="section-heading"><div><span className="eyebrow">Riêng của giáo viên</span><h2>Simulation của tôi</h2></div><Link to="/workspace" className="secondary page-action">Tạo simulation</Link></div><LibraryRows items={mine} assignable /></section>}
