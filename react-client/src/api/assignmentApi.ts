...
import type { Assignment, AssignmentSubmission, CreateAssignment, StudentOption, TeacherClassOption } from "../types/physlive";
...
export const teacherAssignmentClasses = () =>
  axiosClient.get<TeacherClassOption[]>("/assignments/mine/teacher/classes").then(r => r.data);
...
export const gradeAssignmentSubmission = (assignmentId: string, submissionId: string, score: number, feedback: string, confirm = true) =>
  axiosClient.put<AssignmentSubmission>(`/assignments/${assignmentId}/submissions/${submissionId}/grade`, { score, feedback, confirm }).then(r => r.data);
...
export const completeAssignment = (assignmentId: string, request: { conclusion: string; answerText?: string; estimatedValue?: number }) =>
  axiosClient.post<AssignmentSubmission>(`/assignments/${assignmentId}/submit`, request).then(r => r.data);
