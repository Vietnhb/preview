...
export function VersionsTab({ solver }: Readonly<{ solver: boolean }>) {
  ...
  return <div className="modern-card">
    <div className="modern-card-header"><div><h2>{solver ? "Solver versions và reference evidence" : "Topic packs và phê duyệt"}</h2><p>{solver ? "Kiểm tra numerical solver, closed-form/reference solver độc lập và checksum binding trước khi approve." : "Topic pack đã approve là immutable; tạo version mới để thay đổi. Backend kiểm tra meta-schema và các tham chiếu trước khi lưu."}</p></div><button type="button" className="prediction-submit-btn" onClick={() => setEditor({ clone: false })}>+ Tạo bản nháp</button></div>
