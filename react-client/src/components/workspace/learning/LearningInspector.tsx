...
import type { Curriculum, LibraryItem, Problem, Simulation } from "../../../types/physlive";
import { clampControlValue, hasValidControlBounds, interpolateAtTime, isWithinControlBounds, learningSeries, lessonCopy, numberLabel, type LearningControl } from "../../../utils/learningModel";
...
function LearningSavePanel({ folders, topics, modules, levels, lessons, folderId, topicId, moduleId, levelId, lessonId, saveTitle, visibility, saving, saveError, onPersist, onFolderChange, onTopicChange, onModuleChange, onLevelChange, onLessonChange, onSaveTitleChange, onVisibilityChange, onCancelSave }: Readonly<LearningSavePanelProps>) {
  return <form className="learn-save-panel" onSubmit={onPersist}>
    <div className="learn-save-heading"><span className="learn-save-icon"><Icon name="upload" /></span><span><strong>Lưu vào thư viện</strong><small>Bản lưu thuộc tài khoản của bạn và có thể dùng để giao bài.</small></span></div>
...
function LearningExperimentPanel({ copy, controls, initialValues, draft, dirty, error, onParamChange, onResetDraft, onInspectorChange }: Readonly<LearningExperimentPanelProps>) {
  return <>
    ...
    <div className="learn-parameters">{controls.map(control => {
      ...
      const initialValue = Number.isFinite(initialValues[control.key]) ? initialValues[control.key] : value;
      const validBounds = hasValidControlBounds(control);
      const fieldInvalid = !draftValue.trim() || !isWithinControlBounds(control, value);
      const rangeValue = clampControlValue(control, fieldInvalid ? initialValue : value);
      return <div className="learn-parameter" key={control.key}><label htmlFor={`parameter-${control.key}`}><span className="learn-variable">{control.symbol}</span>{control.label}</label><div className="learn-parameter-value"><input id={`parameter-${control.key}`} aria-invalid={fieldInvalid} type="number" inputMode="decimal" step="any" min={control.min} max={control.max} value={draftValue} disabled={!validBounds} onChange={event => onParamChange(control.key, event.target.value)} /><span>{control.unit}</span></div><input aria-label={`Điều chỉnh ${control.label.toLowerCase()}`} type="range" min={control.min} max={control.max} step={control.step} value={rangeValue} disabled={!validBounds} onChange={event => onParamChange(control.key, event.target.value)} /><div className="learn-range-labels"><span>{numberLabel(control.min)}</span><span>{numberLabel(control.max)} {control.unit}</span></div></div>;
    })}{error && <p className="learn-error" role="alert">{error}</p>}</div>
...
type LearningAnalysisPanelProps = Pick<LearningInspectorProps, "copy" | "bottomTab" | "selectedSeries" | "allSeries" | "series" | "validData" | "times" | "time" | "index" | "downloading" | "exportError" | "onBottomTabChange" | "onSelectedSeriesChange" | "onSeek" | "onDownload">;

function LearningAnalysisPanel({ copy, bottomTab, selectedSeries, allSeries, series, validData, times, time, index, downloading, exportError, onBottomTabChange, onSelectedSeriesChange, onSeek, onDownload }: Readonly<LearningAnalysisPanelProps>) {
  return <>
...
export function LearningInspector({ userRole, simulation, savedItem, showSave, onToggleSave, onInspectorChange, ...props }: Readonly<LearningInspectorProps>) {
  return <aside className="learn-inspector" aria-label="Hướng dẫn học và thông số">
    <div className="learn-inspector-nav">
      ...
      {canManageLearning(userRole) && simulation.valid && (savedItem ? <Link className="learn-library-link" to={`/assignments/workspace?libraryItemId=${savedItem.id}`}>Giao bài</Link> : <button type="button" className={`learn-save-button${showSave ? " active" : ""}`} onClick={onToggleSave}>{showSave ? "Đóng" : "Lưu"}</button>)}
    </div>
