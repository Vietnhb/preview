...
export function SpecificationReviewEditor({ specification, loading, ambiguities, onSave, onConfirm }: Readonly<SpecificationReviewEditorProps>) {
  ...
  return <section className="create-chat-specification-review" aria-label="Rà soát specification">
    ...
    <label className="create-chat-specification-end-condition"><span>endCondition (optional)</span><textarea rows={5} value={draft.endCondition} onChange={event => handleChange("endCondition", event.target.value)} spellCheck={false} aria-label="Chỉnh endCondition specification" /></label>
