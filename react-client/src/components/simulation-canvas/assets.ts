...
export type AssetPainter = (ctx: CanvasRenderingContext2D, actor: ActorFrame, palette: CanvasPalette) => void;
export type PropPainter = (ctx: CanvasRenderingContext2D, anchor: Point, angle: number, palette: CanvasPalette) => void;
...
function drawCart(ctx: CanvasRenderingContext2D, actor: ActorFrame, palette: CanvasPalette, orange: boolean) {
  ...
  if (orange) {
    ...
    body.addColorStop(0, "#67e8f9"); body.addColorStop(.42, "#2563eb"); body.addColorStop(1, "#1e3a8a");
  }
  ...
  ctx.beginPath(); ctx.moveTo(-5, -32); ctx.lineTo(7, -23); ctx.stroke();
  for (const wheelX of [-24, 24]) {
    ctx.fillStyle = "#080b12";
    ctx.beginPath(); ctx.arc(wheelX, 17, 11, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#64748b"; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = "#cbd5e1";
    ctx.beginPath(); ctx.arc(wheelX, 17, 6.5, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#334155"; ctx.lineWidth = 1.3;
    for (let spoke = 0; spoke < 4; spoke++) {
      const angle = actor.distance / 4 + spoke * Math.PI / 2;
      ctx.beginPath(); ctx.moveTo(wheelX, 17); ctx.lineTo(wheelX + Math.cos(angle) * 6, 17 + Math.sin(angle) * 6); ctx.stroke();
    }
  }
  if (actor.label) {
    ctx.fillStyle = palette.text; ctx.font = "700 11px Inter, sans-serif"; ctx.textAlign = "center";
...
function drawSportCar(ctx: CanvasRenderingContext2D, actor: ActorFrame) {
  ...
  ctx.fillStyle = "#fef08a"; ctx.fillRect(45, -2, 5, 9);
  ctx.fillStyle = "#fb7185"; ctx.fillRect(-50, -1, 5, 8);
  for (const wheelX of [-27, 28]) {
    ctx.fillStyle = "#05070c"; ctx.beginPath(); ctx.arc(wheelX, 16, 12, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#64748b"; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = "#dbeafe"; ctx.beginPath(); ctx.arc(wheelX, 16, 7, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#334155"; ctx.lineWidth = 1.4;
    for (let spoke = 0; spoke < 5; spoke++) {
      const angle = actor.distance / 4 + spoke * Math.PI * .4;
      ctx.beginPath(); ctx.moveTo(wheelX, 16); ctx.lineTo(wheelX + Math.cos(angle) * 6.5, 16 + Math.sin(angle) * 6.5); ctx.stroke();
    }
  }
  ctx.restore();
}
...
function drawProjectile(ctx: CanvasRenderingContext2D, actor: ActorFrame) {
  ...
  ctx.strokeStyle = "rgba(255,255,255,.9)"; ctx.lineWidth = 1.6; ctx.stroke(); ctx.restore();
}

export const assetRegistry: Record<string, AssetPainter> = {
  "object.cart.blue": (ctx, actor, palette) => drawCart(ctx, actor, palette, false),
  "object.cart.orange": (ctx, actor, palette) => drawCart(ctx, actor, palette, true),
...
function drawTower(ctx: CanvasRenderingContext2D, anchor: Point, _angle: number, palette: CanvasPalette) {
  ...
  gradient.addColorStop(0, "#1e293b"); gradient.addColorStop(.5, "#64748b"); gradient.addColorStop(1, "#172033");
  ctx.fillStyle = gradient; ctx.fillRect(left, top, 42, height);
  ctx.strokeStyle = palette.muted; ctx.lineWidth = 1.3; ctx.strokeRect(left, top, 42, height);
  ctx.strokeStyle = "rgba(255,255,255,.16)";
  for (let y = top; y < bottom - 18; y += 22) {
    ctx.beginPath(); ctx.moveTo(left, y); ctx.lineTo(anchor.x, y + 22); ctx.moveTo(anchor.x, y); ctx.lineTo(left, y + 22); ctx.stroke();
  }
}

function drawCannon(ctx: CanvasRenderingContext2D, anchor: Point, angle: number) {
  ctx.save(); ctx.translate(anchor.x, anchor.y); ctx.rotate(-angle);
  const barrel = ctx.createLinearGradient(0, -7, 45, 7);
  barrel.addColorStop(0, "#334155"); barrel.addColorStop(.5, "#e2e8f0"); barrel.addColorStop(1, "#475569");
  roundedPath(ctx, -2, -7, 45, 14, 4); ctx.fillStyle = barrel; ctx.fill(); ctx.strokeStyle = "#f8fafc"; ctx.stroke();
  ctx.restore();
  ctx.fillStyle = "#0f172a"; ctx.beginPath(); ctx.arc(anchor.x, anchor.y, 10, 0, Math.PI * 2); ctx.fill();
